...
class ModelTree(object):
    ...
    def fit(self, X, y, verbose=False):
        ...
        if verbose:
            print(" max_depth={}, min_samples_leaf={}...".format(
        ...
        def _build_tree(X, y):
            ...
            def _split_traverse_node(node, container):
                ...
                result = _splitter(
                    node, model, max_depth=max_depth, min_samples_leaf=min_samples_leaf)
                ...
                if verbose:
                    depth_spacing_str = " ".join([" "] * node["depth"])
                    print(" {}node {} @ depth {}: loss={:.6f}, j_feature={}, threshold={:.6f}, N=({},{})".format(depth_spacing_str,
                          node["index"], node["depth"], node["loss"], node["j_feature"], node["threshold"], len(X_left), len(X_right)))
